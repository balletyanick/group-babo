...
@section('title', "Liste des prêts en cours")
        ...
        <div class="container-fluid">
            <div class="page-titles">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="javascript:void(0)">Liste des prets en cours   </a></li>
                </ol>
            </div> 
            <!-- row -->
                                    ...
                                    <tbody>
                                        @foreach ($prets as $pret)
                                            <tr>
                                                ...
                                                <td> {{ number_format($pret->amount * 18 / 100, 2, ',', '.') }} FCFA </td>
                                                ...
                                                <td>
                                                    @if(Auth::user()->permission('EDITION PRET') || Auth::user()->permission('SUPPRESSION PRET') || Auth::user()->permission('LISTE MENSUALITE PRET') || Auth::user()->permission('REFUSER PRET') || Auth::user()->permission('VALIDER PRET'))
                                                        <div class="d-flex">

                                                            @if(Auth::user()->permission('EDITION PRET'))
                                                                <a href="{{route('pret.edit',[$pret->id])}}" data-bs-toggle="tooltip" 
                                                                    data-bs-placement="top"  title="Modifier" class="btn btn-primary shadow btn-xs sharp me-1 mr-2">
                                                                    <i class="fa fa-pencil"></i>
                                                                </a>
                                                            @endif 
        
                                                            @if(Auth::user()->permission('SUPPRESSION PRET') )
                                                                <a href="javascript:void(0);" data-bs-toggle="tooltip" 
                                                                    data-bs-placement="top"  title="Supprimer" onclick="deleted('{{$pret->id}}','{{route('pret.delete')}}')" id="icone-delete" class="btn btn-danger shadow btn-xs sharp me-1 mr-2">
                                                            ...
                                                            @if(Auth::user()->permission('VALIDER PRET'))
                                                                <a href="javascript:void(0);" onclick="valider('{{ $pret->id }}', '{{ route('pret.valider', ['id' => $pret->id]) }}')" class="btn btn-success shadow btn-xs sharp me-1 mr-1">
                                                                    <i class="fa fa-check"></i>
                                                                </a>
                                                            @endif 

                                                            @if(Auth::user()->permission('REFUSER PRET'))
                                                                <a href="javascript:void(0);" onclick="refuser('{{$pret->id}}','{{route('pret.refuser', ['id' => $pret->id]) }}')"  class="btn btn-danger shadow btn-xs sharp me-1 mr-1">
